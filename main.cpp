...
int main() {
    ...
    while (true) {      
        ...
        while (word_length <= 0 or word_length > 100) {
        ...
        bool show_total_words = get_yesno("Show running total of words?");
        // prompted for above

        // make vector of total words
        vector<string> words;
        ...
        while (getline(file, tempString)) {
            ...
            if (tempString.size() == word_length){
                words.push_back(tempString);
            }
        }
        game.start_new_game(num_guesses, word_length, words, show_total_words);

        while (!game.is_won() && !game.is_lost()) {
            if (game.get_showWords()){
                cout << "Words in chosen family: " << game.get_family() -> size() << endl;
            }
